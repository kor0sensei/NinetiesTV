...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.StartYear < 2000 && s.EndYear < 2000).ToList();
        }
        ...
        {
            return shows.Where(s => s.Name.StartsWith("The") == true).Select(s => s).ToList();
        }
        ...
        {
            return shows.Where(s => s.Genres.Count > 1).OrderBy(s => s.StartYear).Select(s => s).ToList();
        }
